...
│   │   ├── AgentMatcher.swift       # 에이전트 매칭 도메인 서비스 (ViewModels에서 이동됨)
│   │   ├── BuildQAService.swift     # 빌드/QA 루프 실행 서비스 (Host 콜백 패턴으로 RoomManager 의존성 역전)
│   │   ├── RoomPersistenceService.swift # Room JSON 영속화 서비스 (저장/로드/프루닝)
│   │   └── DomainHintDetector.swift # 도메인 힌트 감지 (JiraDomainDetector에서 rename — detect(text:) + merge() API)
│   ├── ViewModels/
...
│   │   └── ResponseSanitizer.swift  # LLM 응답 정제 유틸리티 (불필요 마크업/메타데이터 제거)
│   ├── Providers/
...
│   │   └── ToolExecutor.swift       # 도구 호출 루프 실행 엔진 (ViewModels에서 이동됨 — 인프라 레이어)
│   ├── Plugins/
│   │   ├── Core/
...
| `DiscussionSession` | 5 | currentRound, isCheckpoint, decisionLog, artifacts, briefing, maxRounds, roundSummaries |
...
**Room Aggregate Root 패턴**:
Room은 DDD Aggregate Root로서 내부 값 객체에 대한 직접 외부 수정을 금지하고, 위임 메서드(delegation method)를 통해서만 상태를 변경한다.
- **private(set) 필드**: `workflowState`, `clarifyContext`, `projectContext`, `discussion`, `buildQA`
- 외부에서는 `room.transitionTo()`, `room.classifyIntent()`, `room.startDiscussion()` 등 도메인 메서드를 호출하여 불변식을 유지
- 값 객체 내부 프로퍼티는 read-only computed wrapper로 접근 (`room.intent`, `room.currentPhase` 등)
...
### ToolExecutor (`Providers/ToolExecutor.swift`)
...
| E-2 | **IntentClassifier + PreIntentRoute**: 규칙 기반 키워드 즉시 분류 → LLM 폴백. `ChatViewModel.handleMasterMessage`에서 `preRoute()`로 Pre-Intent 라우팅 (empty/fileOnly/command/classified/ambiguous). `PreIntentRoute.pendingIntent` — URL만 입력 시 intent nil 유지. derivation 키워드 확장 (도출, 뭘해야, 무슨작업, 작업해야 등). | ✅ |
...
② Assemble ── 3-tier 가중치 에이전트 매칭 (Tier1: skillTags×5, Tier2: workModes+OutputStyle+PositionBonus×2, Tier3: keyword+semantic×3)
              ...
              discussion/research 최소 2명 에이전트 보장 (코드 레벨 가드)
              intent 기반 도메인 보너스 가중치 조절 (discussion/research 50% 감쇠)
...
| research | Understand → Assemble → Design(순차 조사 → 종합) → Deliver |
...
| AgentMatcher | 에이전트 매칭 도메인 서비스 (ViewModels에서 이동됨 — 3-tier 가중치 스코어링) |
| BuildQAService | 빌드/QA 루프 실행 서비스 (Host 콜백 패턴으로 RoomManager 의존성 역전) |
| RoomPersistenceService | Room JSON 영속화 서비스 (저장/로드/프루닝) |
| DomainHintDetector | 도메인 힌트 감지 (JiraDomainDetector에서 rename — Jira 데이터 + 사용자 텍스트 도메인 힌트 병합, detect(text:) + merge() API) |
...
| ViewModels | 302 | ChatViewModel (통합+파싱+상태), AgentStore, ProviderManager, RoomManager, OnboardingViewModel, BuildLoopRunner (QA 포함) |
