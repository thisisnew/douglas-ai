...
**워크플로우 케이스 (구현 시 반드시 준수)**:

| Case | 플로우 |
|------|--------|
| 1. 질의응답 | 프롬프트 → 질의응답 판단 → 방 생성 → 에이전트 매칭+확인(거부→직접선택) → 에이전트 응답 완료 |
| 2. 토론(1인) | 프롬프트 → 토론 판단 → 방 생성 → 에이전트 매칭+확인(거부→직접선택) → 에이전트 응답 완료 |
| 3. 토론(복수) | 프롬프트 → 토론 판단 → 방 생성 → 에이전트 매칭+확인(거부→직접선택) → 에이전트 토론(라운드별 사용자 의견 가능) → DOUGLAS 결론 도출 → 완료 |
| 4. 구현(1인) | 프롬프트 → 구현 판단 → 방 생성 → 에이전트 매칭+확인(거부→직접선택) → 실행 계획 도출 → 승인/요건추가 무한루프 → 실행(실시간 사용자 방향 추가 가능) → **마지막 단계 직전 반드시 승인 요청** → 승인→완료, 거부→지정 단계 롤백 |
| 5. 구현(복수) | 프롬프트 → 구현 판단 → 방 생성 → 에이전트 매칭+확인(거부→직접선택) → 에이전트 토론(라운드별 사용자 의견 가능) → 실행 계획 도출 → 승인/요건추가 무한루프 → 실행(실시간 사용자 방향 추가 가능) → **마지막 단계 직전 반드시 승인 요청** → 승인→완료, 거부→지정 단계 롤백 |
| 6. 방 생성 | Case 1~5와 동일, 에이전트 매칭 단계만 없음 |
| 7. 파일 생성 | 7-1: 진행 중/완료 후 "문서 만들어줘" → 대화 요약 → 문서 생성("완료되었습니다."+경로만 표시). 7-2: 처음부터 "~~문서로 만들어줘" → 조사 후 문서 생성(동일 표시) |
| 8. 후속 처리 | 모든 케이스 완료 후 후속 프롬프트 가능 → 질의응답/토론/구현/파일생성 구분 후 해당 플로우 수행 |
| 9. 요건 불명확 | URL만, 이미지만 등 → DOUGLAS가 재차 질문하여 명확화 |
