...
enum PromptCompositionService {
    ...
    static func researchSynthesisPrompt() -> String {
        ...
        - 마크다운 헤더(##, ###) 최소화. 읽기 좋은 문단 형식으로.
        - 전체 길이는 원본 의견의 절반 이하로 압축하세요.
        - 핵심 요약은 2-3문장, 각 조사 결과는 3문장 이내, 실무 포인트는 한 줄씩 작성하세요.
        """
    }
    ...
    static func discussionRulesBlock() -> String {
        ...
        의견의 근거로 코드를 참조할 때는 도구(code_search, file_read)를 적극 활용하세요.
        단, 토론 중 직접 코드를 수정하거나 파일을 작성하지는 마세요. 분석, 의견, 대안 제시만 하세요.
        도구는 의견의 근거를 위해 사용하세요. 최종 응답은 반드시 2-4문장 의견으로 마무리하세요.
