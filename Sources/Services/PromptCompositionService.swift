...
enum PromptCompositionService {
    ...
    /// 후속 조사 프롬프트 — 다른 전문가의 조사 결과를 보고 도구로 보완 조사
    static func researchFollowUpPrompt() -> String {
        """
        다른 전문가들의 조사 결과를 확인했습니다.
        당신의 프로젝트에서 추가로 확인해야 할 사항이 있으면 도구를 사용하여 보완 조사하세요.

        규칙:
        - 다른 전문가가 찾은 엔드포인트, 파일명, 클래스명 등을 당신의 프로젝트에서 검색하세요.
        - 당신의 1차 조사와 중복되는 내용은 생략하세요. 새로 발견한 내용만 보고하세요.
        - 추가 발견이 없으면 "추가 발견 없음"이라고만 답하세요.
        - 반드시 한국어로 응답하세요.
        """
    }
