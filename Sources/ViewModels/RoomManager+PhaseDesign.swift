        ...
        await withTaskGroup(of: ResearchFinding?.self) { group in
            for agentID in specialists {
                group.addTask { [weak self] () -> ResearchFinding? in
                    ...
                    아래 조사 요청에 대해 **오직 당신의 전문 영역에 해당하는 부분만** 조사하세요.
                    다른 전문가가 조사할 영역은 건드리지 마세요. 결과가 겹치면 품질이 떨어집니다.
                    예: 프론트엔드 → Vue/React 파일, API 호출부. 백엔드 → Java/Repository, 쿼리.
