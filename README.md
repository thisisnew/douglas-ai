...
마스터 에이전트(PM)에게 지시하면, 방(Room)을 만들어 요구사항을 확인하고 전문가를 소환하여 작업을 진행합니다.
...
```
사용자 → 사이드바에 입력 → 마스터(PM)가 즉시 방 생성
  → 요구사항 확인 → 전문가 초대 → 토론 → 계획 → 실행
```

사용자가 에이전트를 직접 고르지 않습니다. 마스터가 PM/오케스트레이터로서 팀을 구성하고 작업을 진행합니다.
...
- **마스터 PM/오케스트레이터** — 사이드바에 입력하면 마스터가 즉시 방 생성, 요구사항 확인, 전문가 소환
- **작업 규칙 (Working Rules)** — 에이전트별 구체적 작업 지시사항 (직접 입력 또는 파일 참조). 에이전트 생성 시 필수 입력
- **필수 토론** — 마스터와 초대된 전문가가 요구사항을 공유하고 접근법 논의
